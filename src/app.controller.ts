...
export class AppController {
  ...
  webshopFormPost(@Body() PaymentDto: PaymentDto,@Res() response : Response) {
    console.log("coupon"+PaymentDto.coupon_code)
    let errors = []
    ...
    if(!/^[A-Z]{2}-\d{4}$/.test(PaymentDto.coupon_code) || !PaymentDto.coupon_code == null){
      errors.push("A kuponkód helytelen")
    }
    if(!/^\d\d\/\d\d$/.test(PaymentDto.exp_date)){
      errors.push("lejárati dátum hibás")
    }
    else if(parseInt(PaymentDto.exp_date.split('/')[1]) < new Date().getFullYear() || parseInt(PaymentDto.exp_date.split('/')[0]) < new Date().getMonth()){
    ...
    response.redirect('paymenSuccess')
